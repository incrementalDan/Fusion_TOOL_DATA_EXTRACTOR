...
import { getVisibleFields } from './fieldVisibility.js';
...
export default function App(){
  ...
  const saveApiKey=v=>{setApiKey(v);localStorage.setItem("anthropicKey",v);};
  ...
  const go=async()=>{
    ...
    try{
      ...
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-allow-browser":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1024,system:buildSYS(),messages})});
  ...
  const ok=apiKey.trim().length>10&&((inputMode==="file"&&!!(fileType==="image"?imgB64:pdfB64))||(inputMode==="text"&&txt.trim().length>10));
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,gap:10,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <span style={{fontSize:10,color:T.sub,whiteSpace:"nowrap"}}>API key</span>
              <div style={{position:"relative",display:"flex",alignItems:"center"}}>
                <input
                  ...
                  placeholder="sk-ant-…"
                  style={{...base,width:160,borderRadius:4,border:`1px solid ${apiKey?T.green:T.amber}`,paddingRight:26,fontSize:11}}
                />
                <button onClick={()=>setShowKey(p=>!p)}
                  style={{position:"absolute",right:4,background:"none",border:"none",cursor:"pointer",color:T.sub,fontSize:11,padding:0,lineHeight:1}}>
                  {showKey?"●":"○"}
                </button>
              </div>
            </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={G}>
            ...
            {isVis("backsideCapable")&&<div style={{...C(3),opacity:isOpt("backsideCapable")?0.55:1}}>
              ...
              <div style={{height:27,display:"flex",alignItems:"center",border:`1px solid ${T.border}`,borderRadius:"0 0 4px 4px",padding:"0 7px",background:T.inputBg}}>
                <Toggle value={F.backsideCapable} onChange={v=>sf("backsideCapable",v)} label={F.backsideCapable?"Yes":"No"}/>
              </div>
            </div>}
            {isVis("doubleEnded")&&<div style={{...C(3),opacity:isOpt("doubleEnded")?0.55:1}}>
              <FL type="proshop" label="Double Ended"/>
              <div style={{height:27,display:"flex",alignItems:"center",border:`1px solid ${T.border}`,borderRadius:"0 0 4px 4px",padding:"0 7px",background:T.inputBg}}>
                <Toggle value={F.doubleEnded} onChange={v=>sf("doubleEnded",v)} label={F.doubleEnded?"Yes":"No"}/>
              </div>
            </div>}
          </div>
          ...
          <div style={G}>
            ...
            {isVis("fullProfile")&&<div style={{...C(4),opacity:isOpt("fullProfile")?0.55:1}}>
              ...
              <div style={{height:27,display:"flex",alignItems:"center",border:`1px solid ${T.border}`,borderRadius:"0 0 4px 4px",padding:"0 7px",background:T.inputBg}}>
                <Toggle value={F.fullProfile} onChange={v=>sf("fullProfile",v)} label={F.fullProfile?"Yes":"No"}/>
              </div>
            </div>}
          </div>
